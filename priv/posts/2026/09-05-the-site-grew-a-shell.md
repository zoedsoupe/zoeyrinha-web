...
Two swaps first. Earmark is out, [mdex](https://github.com/leandrocp/mdex) is in - Earmark had a CVE open and mdex renders via comrak (Rust NIF), so posts compile faster and the CVE is gone. Then I deleted the whole `assets/package.json`: Alpine, Lucide and npm itself, all gone. The site is a terminal with a Dracula palette, it never needed a JS framework. The icons became text links, the sprinkle of interactivity is now ~80 lines of vanilla JS, and the Docker image shrank accordingly.
